...
def embedding_bidirectional_attention_seq2seq(encoder_inputs,
                                decoder_inputs,
                                cell_fw,
                                cell_bw,
                                cell,
  ...
  Returns:
    A tuple of the form (outputs, state), where:
      ...
      state: The state of each decoder cell at the final time-step.
        It is a 2D Tensor of shape [batch_size x cell.state_size]."""
  with variable_scope.variable_scope(
      scope or "embedding_bidirectional_attention_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
    # Encoder.
    encoder_cell_fw = rnn_cell.EmbeddingWrapper(cell_fw, embedding_classes=num_encoder_symbols,embedding_size=embedding_size)
    encoder_cell_bw = rnn_cell.EmbeddingWrapper(cell_bw, embedding_classes=num_encoder_symbols,embedding_size=embedding_size)
    # encoder_outputs, encoder_state = rnn.rnn(encoder_cell, encoder_inputs, dtype=dtype)
    # encoder_outputs, encoder_state_fw, encoder_state_bw = rnn.bidirectional_rnn(cell_fw, cell_bw, encoder_inputs, dtype=dtype)
    
    encoder_outputs, encoder_state_fw = rnn.rnn(encoder_cell_fw, encoder_inputs, dtype=dtype)

    with variable_scope.variable_scope("backwards_encoder"):
      encoder_outputs_bw, encoder_state_bw = rnn.rnn(encoder_cell_bw, list(reversed(encoder_inputs)), dtype=dtype)

    if num_layers > 1:
      encoder_state = []
      for layer in range(num_layers):
        enc_c = array_ops.concat(1, (encoder_state_fw[layer].c, encoder_state_bw[layer].c))
        enc_h = array_ops.concat(1, (encoder_state_fw[layer].h, encoder_state_bw[layer].h))
        encoder_state.append(rnn_cell.LSTMStateTuple(c = enc_c,h = enc_h))
      print("Encoder States Concanated")
    elif num_layers==1:
      enc_c = tf.concat((encoder_state_fw.c, encoder_state_bw.c), 1, name ='bidirectional_concat_c')
      enc_h = tf.concat((encoder_state_fw.h, encoder_state_bw.h), 1, name ='bidirectional_concat_h')
      encoder_state = LSTMStateTuple(c=encoder_state_c, h=encoder_state_h)

    # if use_lstm:
    #   encoder_state_c, encoder_state_h = array_ops.split(1, 2, encoder_state)
    #   encoder_state_bw_c, encoder_state_bw_h = array_ops.split(1, 2, encoder_state_bw)
    #   encoder_state = array_ops.concat(1, [encoder_state_c, encoder_state_bw_c, encoder_state_h, encoder_state_bw_h])
    # else:
    #   encoder_state = array_ops.concat(1, [encoder_state, encoder_state_bw])
    encoder_outputs = [array_ops.concat(1, [encoder_outputs[i], encoder_outputs_bw[i]]) for i in range(0, len(encoder_outputs))]

    # First calculate a concatenation of encoder outputs to put attention on.
    top_states = [array_ops.reshape(e, [-1, 1, 2*cell_fw.output_size])
                  for e in encoder_outputs]
    attention_states = array_ops.concat(1, top_states)
...
def pretrained_embedding_bidirectional_attention_seq2seq(encoder_inputs,
                                decoder_inputs,
                                cell_fw,
                                cell_bw,
                                cell,
                                ...
                                initial_state_attention=False,
                                use_lstm = 1):
  """Pretrained Embedding sequence-to-sequence model with attention.
